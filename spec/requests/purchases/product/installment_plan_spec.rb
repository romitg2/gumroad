...
describe "Product with installment plan", type: :system, js: true do
  ...
  describe "tips" do
    it "charges the full tip amount on the first payment" do
      ...
      expect(page).to have_text("Subtotal US$10", normalize_ws: true)
      expect(page).to have_text("Tip US$1.50", normalize_ws: true)
      expect(page).to have_text("Total US$11.50", normalize_ws: true)
      expect(page).to have_text("Payment today US$4.84", normalize_ws: true)
      expect(page).to have_text("Future installments US$6.66", normalize_ws: true)
      ...
      expect(purchase.tip.value_cents).to eq(150)
      expect(subscription).to have_attributes(
        is_installment_plan: true,
        charge_occurrence_count: 3,
        recurrence: "monthly",
